...
let playerSpeed = 8; // Increased player speed
let gravity = 1.8; // Adjusted gravity
let calculatedGravity = 3.0; // Original gravity for platform generation
let highScore = 0;
let backgroundImage;
let cameraOffset = 0;
...
let jumpStrength = 25; // Adjusted jump strength
let calculatedJumpStrength = 27; // Original jump strength for platform generation
...
function draw() {
  ...
  if (isGameOver) {
    ...
    // Center the play again button below the Game Over text
    playAgainButton.x = (width - playAgainButton.width) / 2;
    playAgainButton.y = (height / 2) + 50; // Adjust this value to set the distance between the text and the button
  ...
  platforms.forEach(platform => {
    ...
    let radius = 10; // Radius for the circular edges
  
    // Draw the platform with rounded corners
    rect(platformX, platform.y, platform.width, platform.height, radius);
    ...
    rect(platformX, platform.y, platform.width, platform.height, radius); // Rounded hitbox outline
    noStroke();
  });
...
function generatePlatform(lastX) {
  ...
  // Set minimum and maximum distances to 115% and 125% of the calculated jumpWidth and jumpHeight respectively
